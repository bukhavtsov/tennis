...
SELECT first_name, last_name, country.name AS country FROM sportsmen INNER JOIN country ON sportsmen.country_id = country.id WHERE country.name in ('The Russian Federation', 'Argentine Republic');

--Query returns stadium name and country from The Federal Republic of Germany or The Russian Federation
SELECT * FROM stadium INNER JOIN country ON stadium.country_id = country.id WHERE country.name = 'The Federal Republic of Germany' or country.name like '%Russian%';
